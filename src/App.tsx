...
function App() {
  ...
  const [games, setGames] = useState<{ id: string; a: string; b: string; lines: number; winner: string }[]>([])
  ...
  useEffect(() => {
    try {
      ...
      const savedGames = JSON.parse(localStorage.getItem('bst:games') || '[]') as { id: string; a: string; b: string; lines: number; winner: string }[]
